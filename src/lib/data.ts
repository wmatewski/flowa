...
export const getLiveSessionDataById = async (
  sessionId: string,
  limit = 25,
): Promise<PublicLiveSessionData> => {
  const supabase = createSupabaseAdminClient();
...
export const getLiveSessionDataForAccess = async (
  access: SessionAccessContext,
  sessionId: string,
  limit = 25,
): Promise<PublicLiveSessionData> => {
  ...
  const supabase = createSupabaseAdminClient();
  const [sessionResult, overviewResult, participantResult] = await Promise.all([
    supabase
      .from("sessions")
      .select(sessionColumns)
      .eq("id", sessionId)
